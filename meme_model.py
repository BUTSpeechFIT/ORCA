...
def bernoulli_kl(logits, labels):
    ...
    if labels is not None:
        ...
        labels1 = labels1.sum(dim=-1, keepdim=True) / labels1_mask.sum(dim=-1, keepdim=True).clamp(min=1e-8)  # Avoid division by zero
        labels0 = 1 - labels1
...
def beta_llh(params, labels, eps=1e-2):
    ...
    return (negative_log_probs,
            ...
            (concentration0*concentration1)/((concentration1 + concentration0 + 1) * ((concentration1 + concentration0)**2)).to(original_dtype)
            )
...
def beta_moment_matching(params, labels, eps=1e-2):
    ...
    if labels is not None:
        ...
        predicted_variance = (concentration0 * concentration1) / ((concentration1 + concentration0 + 1) * ((concentration1 + concentration0)**2))
        moments_difference = (predicted_mean - labels_mean) ** 2 + (predicted_variance - labels_variance) ** 2
    else:
        moments_difference = None
    return (moments_difference,
            (concentration1/(concentration1 + concentration0)).to(original_dtype),
            (concentration0*concentration1)/((concentration1 + concentration0 + 1) * ((concentration1 + concentration0)**2)).to(original_dtype)
            )
...
class Meme(torch.nn.Module):
    ...
    def forward(self, x, prior_params=None):
        ...
        hidden_states = [hd[torch.arange(hd.shape[0]), input_lengths - 1, :] for hd in hidden_states]
    ...
    def load_from_directory(cls, dir_path, lm, device=None):
        ...
        model.load_state_dict(
            torch.load(os.path.join(dir_path, 'model_minus_lm.pt'),
                       map_location=device,
                       weights_only=True),
            strict=False
        )
